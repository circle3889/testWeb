//function goBannerHTTPS(link, billyn, inout, static_cd){
//		
//		if( "Y" == billyn ){
//			if(confirm("3G/LTE망 사용 시 데이터 요금이 발생됩니다.")){
//				callCSScript('Z1605311813129055', '7216540007', static_cd , '', '');
//				if(inout == "OUT") {
//	 				if(link.search("https://") > -1) location.href = "toapp:webbrowserouter:" + "m2.tworld.co.kr/jsp/iphoneredirectPage.jsp?URL=" + encodeURIComponent(link);
//	 				else {
//	 					link = link.replace("http://","");
//	 					if(link.search("https://") > -1) location.href = "toapp:webbrowserouter:" + link;
//	 				}
//	 			}
...
Banners = {
	getBannerData : function(opt){
		var gaca = "MTA_main";
		var gaac = "메인 배너"; 
		$.post("/json/main_banner.json", function(data) {
...
//				var randTime0 = Math.floor(Math.random() * (255 - 0 + 1)) + 0;
//				var randTime1 = Math.floor(Math.random() * (255 - 0 + 1)) + 0;
//				var randTime2 = Math.floor(Math.random() * (255 - 0 + 1)) + 0;
//				var randColor = "rgb(" +  randTime0 + "," + randTime1 + "," + randTime2 + ")"
 				if (MobileUA.IOS)  {
...
//						    		   + randColor + '"'
						    	...
						    	banner = '<div class="swiper-slide" '
						    		   +'>'
						    		   + '<a href="javascript:onAfterBinding.goBanner(\''
						    		   + t_link
						    		   + "', '"
							    	   + value.b_billyn
							    	   + "', '"
