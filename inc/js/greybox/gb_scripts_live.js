...
var _2=window.location.hostname.indexOf("m");
var _3=this.src_loader.indexOf("m");
if(_2!=-1&&_3==-1){
this.src_loader=this.src_loader.replace("://","://m.");
}
if(_2==-1&&_3!=-1){
this.src_loader=this.src_loader.replace("://m.","://");
}
...
if(_15=="gb_live_center"){
a.onclick=function(){
var sp=_16.split(/, ?/);
GB_showLiveCenter(_18.caption,_18.url,parseInt(sp[1]),parseInt(sp[0]));
return false;
};
}
}
...
GB_LiveWindow = GreyBox.extend({
	init:function(_56) {
		this.parent({});
		//this.img_header=this.root_dir+"header_bg.gif";
		this.img_close=this.root_dir+"blank.gif";
		this.show_close_img=true;
		AJS.update(this,_56);
		this.addCallback(this.callback_fn);
	},
	initHook:function() {
	AJS.addClass(this.g_window,"GB_WindowLive");
	this.header=AJS.TABLE({"class":"headerlive"});
	this.header.style.backgroundImage="url("+this.img_header+")";
	var _57=AJS.TD({"class":"caption"},this.caption);
	var _58=AJS.TD({"class":"closelive"});
	if(this.show_close_img){
		var _59=AJS.IMG({"src":this.img_close, "alt":"´Ý±â"});
		//var _5a=AJS.SPAN("Close");
		var btn=AJS.DIV(_59);
		AJS.AEV([_59],"mouseover",function() {
			AJS.addClass(_59,"on");
		});
		AJS.AEV([_59],"mouseout",function(){
			AJS.removeClass(_59,"on");
		});
		AJS.AEV([_59],"mousedown",function(){
			AJS.addClass(_59,"click");
		});
		AJS.AEV([_59],"mouseup",function(){
...
AJS.preloadImages(GB_ROOT_DIR+"blank.gif",GB_ROOT_DIR+"header_bg.gif");
